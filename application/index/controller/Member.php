...
//            step 3.验证账号密码正不正确
            $member =$member->where('email',$email)->where('password',md5($password))->find();
            if (!$member) {
                //密码错误
                return json_encode(
                    [
            ...
            $username=$member->query('select username from tp_member where email="'.$email.'" limit 1');
            $username=$username[0]['username'];
            $userid=$member->query('select userid from tp_member where email="'.$email.'" limit 1');
